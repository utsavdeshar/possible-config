SELECT
  SUM(If(age >= 6 AND age <= 11, vitaminA, 0))  AS '6-11M Vitamin-A',
  SUM(If(age >= 12 AND age <= 59, vitaminA, 0)) AS '12-59M Vitamin-A',
  SUM(deworming)                                AS 'Deworming tablet'
FROM (
  SELECT
    p.person_id,
  ...
  WHERE !p.voided AND !v.voided AND !e.voided And !oVitaminA
