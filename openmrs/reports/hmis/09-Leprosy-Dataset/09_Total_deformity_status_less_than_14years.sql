...
FROM
    ...
    LEFT JOIN concept_name answer_concept_short_name ON ca.answer_concept = answer_concept_short_name.concept_id
        ...
        IS FALSE
    WHERE
        question_concept_name.name IN ('Leprosy-Disability grade')
            AND cd.name = 'Coded'
    ...
    (SELECT 'M' AS gender UNION SELECT 'F' AS gender) gender
        INNER JOIN
    reporting_age_group rag ON rag.report_group_name = 'Leprosy Control Deformity'
    ...
    INNER JOIN concept_name cn1 ON o1.concept_id = cn1.concept_id
        ...
        AND cn1.name = 'Leprosy-Disability grade'
        AND o1.voided = 0
        AND cn1.voided = 0
    ...
    person p ON first_concept.person_id = p.person_id
        ...
        AND rag.report_group_name = 'Leprosy Control Deformity'
