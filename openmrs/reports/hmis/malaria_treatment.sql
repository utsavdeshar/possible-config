SELECT
  malaria_types_list.answer_concept_name                                                               AS 'Type of Malaria note',
...
FROM visit
  ...
  INNER JOIN coded_obs_view AS malaria_type ON encounter.encounter_id = malaria_type.encounter_id
                                   AND malaria_type.concept_full_name = 'Malaria note-Finding'
  ...
  LEFT OUTER JOIN obs_view AS malaria_treatment ON malaria_treatment.obs_group_id = malaria_type.obs_group_id
                                   AND malaria_treatment.concept_full_name = 'Malaria note-Treatment start date'
  ...
  RIGHT OUTER JOIN (SELECT answer_concept_name
                      ...
                      WHERE question_concept_name = 'Malaria note-Finding') AS malaria_types_list
