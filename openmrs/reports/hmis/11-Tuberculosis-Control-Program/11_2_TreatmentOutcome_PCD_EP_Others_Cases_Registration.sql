SELECT 
    second_answers.answer_name AS 'Diagonasis',
...
FROM
    ...
    INNER JOIN visit v1 ON v1.visit_id = e.visit_id
    WHERE       
    CAST(v1.date_started AS DATE) BETWEEN DATE(DATE_SUB('#startDate#', INTERVAL 12 MONTH)) AND DATE(DATE_SUB('#endDate#', INTERVAL 12 MONTH)))second_concept ON second_concept.answer = second_answers.answer
    ...
    INNER JOIN concept_name cn2 ON o1.value_coded = cn2.concept_id
        AND cn2.concept_name_type = 'FULLY_SPECIFIED'
        AND cn2.name NOT IN ('New diagnosis')
        AND cn2.voided = 0
    INNER JOIN encounter e ON o1.encounter_id = e.encounter_id
    INNER JOIN visit v1 ON v1.visit_id = e.visit_id
    WHERE
...
GROUP BY second_answers.answer_name,gender.gender;
