...
FROM
    ...
    INNER JOIN concept_name answer_concept_fully_specified_name ON ca.answer_concept = answer_concept_fully_specified_name.concept_id
        ...
        AND answer_concept_fully_specified_name.name IN ('Pulmonary CD' , 'Pulmonary BC')
    ...
    INNER JOIN concept_name cn2 ON o1.value_coded = cn2.concept_id
        ...
        AND cn2.name IN ('Pulmonary CD' , 'Pulmonary BC')
        AND cn2.voided = 0
    ...
    INNER JOIN concept_name cn2 ON o1.value_coded = cn2.concept_id
        AND cn2.concept_name_type = 'FULLY_SPECIFIED'
        AND cn2.name IN ('Relapse','Other previously treated', 'Treatment after failure','Transfer in','Treatment after loss to follow up','Previous treatment history unknown')
        AND cn2.voided = 0
    INNER JOIN encounter e ON o1.encounter_id = e.encounter_id
    INNER JOIN visit v1 ON v1.visit_id = e.visit_id
    WHERE
